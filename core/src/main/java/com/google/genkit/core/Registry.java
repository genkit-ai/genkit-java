...
/**
 * Registry holds all registered actions and associated types, and provides methods to register,
 * query, and look up actions.
 *
 * <p>The Registry is the central component for managing Genkit primitives. It provides:
 *
 * <ul>
 *   <li>Storage and lookup of actions by key
 *   <li>Plugin management
 *   <li>Value storage for configuration
 *   <li>Schema registration for JSON validation
 *   <li>Hierarchical registry support for scoped operations
 * </ul>
 */
public interface Registry {

  /**
   * Creates a new child registry that inherits from this registry. Child registries are useful for
   * scoped operations and will fall back to the parent for lookups if a value is not found in the
   * child.
   *
   ...
   * @throws IllegalStateException if a plugin with the same name is already registered
   */
   ...
   *
   * @param key the action key (type + name)
   * @param action the action to register
   * @throws IllegalStateException if an action with the same key is already registered
   */
   ...
   * @param schema the schema as a map
   * @throws IllegalStateException if a schema with the same name is already registered
   */
  void registerSchema(String name, Map<String, Object> schema);

  /**
   * Returns the plugin for the given name. It first checks the current registry, then falls back to
   * the parent if not found.
   *
   * @param name the plugin name
  ...
  /**
   * Returns the action for the given key. It first checks the current registry, then falls back to
   * the parent if not found.
   *
  ...
  /**
   * Returns the value for the given name. It first checks the current registry, then falls back to
   * the parent if not found.
   *
   * @param name the value name
  ...
  /**
   * Looks up an action by key. If the action is not found, it attempts dynamic resolution through
  ...
  /**
   * Returns a list of all registered actions. This includes actions from both the current registry
