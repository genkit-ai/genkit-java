...
/**
 * Sample application demonstrating Google GenAI (Gemini) integration with Genkit.
 *
 * <p>This sample uses the Jetty plugin to expose flows via HTTP endpoints, allowing you to use the
 * Genkit Developer UI.
 *
 * <p>To run this sample:
 *
 * <ol>
 *   <li>Set the GOOGLE_API_KEY environment variable with your Gemini API key
 *   <li>Run: genkit start -- ./run.sh
 *   <li>Open the Genkit Developer UI at http://localhost:4000
 * </ol>
 */
public class GoogleGenAIApp {
  ...
  public static void main(String[] args) throws Exception {
    ...
    System.out.println(
        "  - imageGeneration: Generate images with Imagen (saves to " + OUTPUT_DIR + "/)");
    ...
    genkit.defineFlow(
        ...
        (ctx, prompt) -> {
          GenerationConfig config =
              GenerationConfig.builder().temperature(0.7).maxOutputTokens(500).build();
            ...
            for (Part part : response.getMessage().getContent()) {
              if (part.getMedia() != null) {
                ...
                if (url.startsWith("data:")) {
                  ...
                  String extension =
                      contentType != null && contentType.contains("png") ? "png" : "jpg";
                  String filename =
                      "image_" + System.currentTimeMillis() + "_" + imageCount + "." + extension;
  ...
  private static void defineTextToSpeechFlow(Genkit genkit) {
    genkit.defineFlow(
        ...
        String.class,
        (ctx, text) -> {
          Map<String, Object> ttsOptions =
              Map.of(
                  "voiceName", "Zephyr" // Available: Zephyr, Puck,
                  // Charon, Kore,
                  // Fenrir, etc.
                  );
            ...
            for (Part part : response.getMessage().getContent()) {
              if (part.getMedia() != null) {
                ...
                if (url.startsWith("data:")) {
                  ...
                  String filename =
                      "audio_" + System.currentTimeMillis() + "_" + audioCount + "." + extension;
            ...
            for (Part part : response.getMessage().getContent()) {
              if (part.getMedia() != null) {
                ...
                if (url.startsWith("data:")) {
                  ...
                  String filename =
                      "video_" + System.currentTimeMillis() + "_" + videoCount + "." + extension;
