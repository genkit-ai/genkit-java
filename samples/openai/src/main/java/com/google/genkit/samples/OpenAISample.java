 ...
 *
 * <p>This example shows how to: - Configure Genkit with the OpenAI plugin - Define flows - Use
 * tools - Generate text with OpenAI models - Expose flows via HTTP endpoints - Process images with
 * vision models - Generate images with DALL-E
 *
...
public class OpenAISample {
  ...
  public static void main(String[] args) throws Exception {
              ...
              ModelResponse response =
                  genkit.generateStream(
                      GenerateOptions.builder()
                          .model("openai/gpt-4o")
                          .system(
                              "You are a helpful assistant that provides detailed, comprehensive responses.")
                          .prompt(userMessage)
    ...
    Flow<String, String, Void> streamingWeatherFlow =
        genkit.defineFlow(
            "streamingWeather",
            String.class,
            String.class,
              ...
              ModelResponse response =
                  genkit.generateStream(
                      GenerateOptions.builder()
                          ...
                          .prompt(userMessage)
                          .tools(List.of(weatherTool))
                          .config(GenerationConfig.builder().maxOutputTokens(500).build())
                          .build(),
                      (chunk) -> {
    ...
    Flow<String, String, Void> describeImageFlow =
        genkit.defineFlow(
            "describeImage",
            String.class,
            String.class,
            (ctx, imageUrl) -> {
              ...
              userMessage.setContent(
                  List.of(
                      ...
                      Part.media(
                          "image/jpeg", imageUrl) // Can also be image/png, image/gif, image/webp
                      ));
    ...
    // URI)
    Flow<String, String, Void> generateImageFlow =
        genkit.defineFlow(
            "generateImage",
            String.class,
            String.class,
            (ctx, prompt) -> {
              ...
              ModelResponse response =
                  genkit.generate(
                      GenerateOptions.builder()
                          .model("openai/dall-e-3") // DALL-E
                          // 3 for
                          // image
                          // generation
                          .prompt(prompt)
                          .config(GenerationConfig.builder().custom(imageOptions).build())
                          .build());
              ...
              for (Part part : parts) {
                if (part.getMedia() != null) {
                  ...
                  if (imageUrl.startsWith("data:")) {
                    System.out.println(
                        "Image returned as base64 data URI (length: "
                            + imageUrl.length()
                            + " chars)");
                  } else {
    ...
    System.out.println(
        "  POST http://localhost:8080/api/flows/streamingWeather (uses streaming + tools)");
    System.out.println(
        "  POST http://localhost:8080/api/flows/describeImage (vision - analyze images)");
    System.out.println(
        "  POST http://localhost:8080/api/flows/generateImage (DALL-E - generate images)");
    System.out.println("");
    ...
    System.out.println(
        "  # Analyze an image with GPT-4 Vision (use a direct image URL, not a webpage):");
    System.out.println(
