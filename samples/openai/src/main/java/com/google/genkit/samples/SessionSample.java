 ...
 *
 * <p>This example shows how to: - Create sessions with persistent state - Conduct multi-turn
 * conversations with automatic history management - Use multiple conversation threads within a
 * session - Implement custom session stores for persistence - Use tools within session-based chats
 *
 * <p>To run: 1. Set the OPENAI_API_KEY environment variable 2. Run: mvn exec:java
...
public class SessionSample {
  ...
  public static void main(String[] args) throws Exception {
    ...
    Tool<Map<String, Object>, Map<String, Object>> reminderTool =
        genkit.defineTool(
            "setReminder",
            ...
            Map.of(
                ...
                Map.of(
                    "message",
                    Map.of("type", "string", "description", "The reminder message"),
                    "time",
                    Map.of(
                        "type",
                        "string",
            ...
            (ctx, input) -> {
              ...
              result.put(
                  "message", "Reminder set: " + input.get("message") + " at " + input.get("time"));
              return result;
            });
  ...
  private static void sessionWithStateExample(Genkit genkit) throws Exception {
    ...
    Session<UserState> session =
        genkit.createSession(
            SessionOptions.<UserState>builder().initialState(new UserState("Alice")).build());
    ...
    Chat<UserState> chat =
        session.chat(
            ChatOptions.<UserState>builder()
                .model("openai/gpt-4o-mini")
                .system(
                    "You are a helpful assistant. The user's name is "
                        + session.getState().getUserName()
                        + ".")
                .build());
  ...
  private static void multiThreadExample(Genkit genkit) throws Exception {
    ...
    Chat<Void> codingChat =
        session.chat(
            "coding",
            ChatOptions.<Void>builder()
                .model("openai/gpt-4o-mini")
                .system("You are an expert programmer. Provide concise code examples.")
                .build());
    ...
    System.out.println(
        "Response: "
            + generalResponse
                .getText()
                .substring(0, Math.min(100, generalResponse.getText().length()))
            + "...\n");
    ...
    System.out.println(
        "Response: "
            + codingResponse
                .getText()
                .substring(0, Math.min(100, codingResponse.getText().length()))
            + "...\n");
    ...
    System.out.println(
        "Response: "
            + followUp.getText().substring(0, Math.min(100, followUp.getText().length()))
            + "...\n");
  }

  /** Demonstrates using tools within session-based chats. */
  private static void sessionWithToolsExample(Genkit genkit, Tool<?, ?> reminderTool)
      throws Exception {
    ...
    Chat<Void> chat =
        session.chat(
            ChatOptions.<Void>builder()
                ...
                .tools(List.of((Tool<?, ?>) reminderTool))
                .build());
  ...
  private static void sessionPersistenceExample(Genkit genkit) throws Exception {
    ...
    Session<UserState> session =
        genkit.createSession(
            SessionOptions.<UserState>builder()
                .store(store)
                .sessionId("persistent-session-001")
                .initialState(new UserState("Bob"))
                .build());
    ...
    Session<UserState> loadedSession =
        genkit
            .loadSession(
                "persistent-session-001", SessionOptions.<UserState>builder().store(store).build())
            .get();
