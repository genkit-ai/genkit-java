 ...
 *
 * <p>This example shows how to: - Configure Genkit with the Groq plugin - Use Llama models with
 * ultra-fast inference - Use Mixtral for high-quality responses - Use Gemma 2 for efficient
 * processing - Define flows with tool usage - Leverage Groq's speed for real-time applications
 *
 * <p>To run: 1. Set the GROQ_API_KEY environment variable 2. Run: mvn exec:java
 */
public class GroqSample {
  ...
  public static void main(String[] args) throws Exception {
    ...
    Flow<String, String, Void> greetingFlow =
        genkit.defineFlow(
            "greeting", String.class, String.class, (name) -> "Hello from Groq, " + name + "!");
              ...
              ModelResponse response =
                  genkit.generate(
                      GenerateOptions.builder()
                          ...
                          .system(
                              "You are a helpful AI assistant providing fast, accurate responses powered by Groq's ultra-fast inference.")
                          .prompt(userMessage)
                          .build());
    ...
    Tool<Map<String, Object>, Map<String, Object>> timezoneTool =
        genkit.defineTool(
            "convertTimezone",
            ...
            Map.of(
                ...
                Map.of(
                    "time",
                    Map.of(
                        "type", "string", "description", "The time to convert (e.g., '3:00 PM')"),
                    "fromZone",
                    Map.of("type", "string", "description", "Source timezone (e.g., 'PST')"),
                    "toZone",
                    Map.of("type", "string", "description", "Target timezone (e.g., 'EST')")),
                "required",
                new String[] {"time", "fromZone", "toZone"}),
    ...
    Flow<String, String, Void> timeAssistantFlow =
        genkit.defineFlow(
            "timeAssistant",
            String.class,
            String.class,
              ...
              ModelResponse response =
                  genkit.generate(
                      GenerateOptions.builder()
                          .model("groq/llama-3.3-70b-versatile")
                          .system(
                              "You are a helpful time zone assistant. Use the convertTimezone tool when needed.")
                          .prompt(userMessage)
                          .tools(List.of(timezoneTool))
                          .build());
    ...
    Flow<String, String, Void> fastStreamingFlow =
        genkit.defineFlow(
            "fastStreaming",
            String.class,
            String.class,
              ...
              ModelResponse response =
                  genkit.generateStream(
                      GenerateOptions.builder()
                          ...
                          .system(
                              "You are a fast, helpful assistant. Provide concise, accurate responses.")
                          .prompt(userMessage)
    ...
    Flow<String, String, Void> qualityChatFlow =
        genkit.defineFlow(
            "qualityChat",
            String.class,
            String.class,
    ...
    Flow<String, String, Void> realTimeQAFlow =
        genkit.defineFlow(
            "realTimeQA",
            String.class,
            String.class,
    ...
    Flow<String, String, Void> speedComparisonFlow =
        genkit.defineFlow(
            "speedComparison",
            String.class,
            String.class,
              ...
              ModelResponse response1 =
                  genkit.generate(
                      GenerateOptions.builder()
                          .model("groq/llama-3.1-8b-instant")
                          .prompt(prompt)
                          .config(GenerationConfig.builder().maxOutputTokens(200).build())
                          .build());
