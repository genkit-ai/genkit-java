...
/**
 * Sample application demonstrating Genkit evaluations, evaluators, and datasets.
 *
 * <p>This example shows how to: - Define custom evaluators - Create and manage datasets - Run
 * evaluations - Use LLM-based evaluators
 *
 * <h2>To run with Dev UI:</h2>
 *
 * <ol>
 *   <li>Set the OPENAI_API_KEY environment variable (for LLM-based evaluators)
 *   <li>Navigate to the sample directory: {@code cd java/samples/evaluations}
 *   <li>Run the app: {@code mvn exec:java}
 *   <li>In a separate terminal, from the same directory, run: {@code genkit start}
 *   <li>Open the Dev UI at http://localhost:4000
 * </ol>
 *
 * <p><b>Important:</b> Run {@code genkit start} from the same directory where the Java app is
 * running. This ensures the Dev UI can find the datasets stored in {@code .genkit/datasets/}.
 */
public class EvaluationsSample {
  ...
  public static void main(String[] args) throws Exception {
    ...
    Flow<String, String, Void> describeFood =
        genkit.defineFlow(
            "describeFood",
            String.class,
            String.class,
                    ...
                    genkit.generate(
                        GenerateOptions.builder()
                            ...
                            .prompt(
                                "Describe "
                                    + food
                                    + " in a delicious and appetizing way in 2-3 sentences.")
                            .config(
    ...
    Evaluator<Void> lengthEvaluator =
        genkit.defineEvaluator(
            "custom/length",
            "Output Length",
            "Evaluates whether the output has an appropriate length (50-500 characters)",
            (dataPoint, options) -> {
              ...
              if (length >= 50 && length <= 500) {
                ...
                reasoning =
                    "Output too short (" + length + " chars). Expected at least 50 characters.";
              } else {
                score = Math.max(0, 1.0 - (length - 500) / 500.0);
                status = EvalStatus.FAIL;
                reasoning =
                    "Output too long (" + length + " chars). Expected at most 500 characters.";
              }

              return EvalResponse.builder()
                  .testCaseId(dataPoint.getTestCaseId())
                  .evaluation(
                      Score.builder().score(score).status(status).reasoning(reasoning).build())
                  .build();
            });

    // Keyword presence evaluator
    Evaluator<Void> keywordEvaluator =
        genkit.defineEvaluator(
            "custom/keywords",
            "Food Keywords",
            "Checks if the output contains food-related descriptive keywords",
            (dataPoint, options) -> {
              String output =
                  dataPoint.getOutput() != null
                      ? dataPoint.getOutput().toString().toLowerCase()
                      : "";
    ...
    Evaluator<Void> sentimentEvaluator =
        genkit.defineEvaluator(
            "custom/sentiment",
            "Positive Sentiment",
            "Evaluates whether the output has a positive/appetizing sentiment",
            (dataPoint, options) -> {
              String output =
                  dataPoint.getOutput() != null
                      ? dataPoint.getOutput().toString().toLowerCase()
              ...
              for (String word : positiveWords) {
                if (output.contains(word)) positiveCount++;
              }
              for (String word : negativeWords) {
                if (output.contains(word)) negativeCount++;
              }
    ...
    Evaluator<Void> deliciousnessEvaluator =
        genkit.defineEvaluator(
            "custom/deliciousness",
            "Deliciousness",
            "Uses an LLM to evaluate how delicious and appetizing the description sounds",
            true, // isBilled - this
            // evaluator
            // makes LLM
            // calls
            ...
            (dataPoint, options) -> {
              ...
              try {
                ...
                if (responseText.contains("\"score\"")) {
                  ...
                  if (scoreEnd == -1) scoreEnd = responseText.indexOf("}", scoreStart);
                  try {
                ...
                return EvalResponse.builder()
                    ...
                    .evaluation(
                        Score.builder()
                            .score(score)
                            .status(score >= 0.6 ? EvalStatus.PASS : EvalStatus.FAIL)
                            .reasoning(reasoning)
                            .build())
                    .build();
                ...
                return EvalResponse.builder()
                    ...
                    .evaluation(
                        Score.builder()
                            .error("Failed to evaluate: " + e.getMessage())
    ...
    boolean datasetExists =
        existingDatasets.stream().anyMatch(d -> "food_descriptions".equals(d.getDatasetId()));
      ...
      List<DatasetSample> samples =
          Arrays.asList(
              ...
              DatasetSample.builder()
                  ...
                  .input("sushi")
                  .reference(
                      "Fresh, delicate rolls of vinegared rice with raw fish and vegetables.")
                  .build(),
              DatasetSample.builder()
                  .testCaseId("food_3")
                  .input("tacos")
              ...
              DatasetSample.builder()
                  ...
                  .reference("Rich, moist layers of chocolate with creamy frosting.")
                  .build(),
              DatasetSample.builder()
                  .testCaseId("food_5")
                  .input("ramen")
    ...
    Flow<String, Map<String, Object>, Void> runEvaluationFlow =
        genkit.defineFlow(
            "runEvaluation",
            String.class,
              ...
              try {
                ...
                RunEvaluationRequest request =
                    RunEvaluationRequest.builder()
                        ...
                        .targetAction("/flow/describeFood")
                        .evaluators(
                            Arrays.asList("custom/length", "custom/keywords", "custom/sentiment"))
                        .build();
