...
package com.google.genkit.samples.chroma;
...
/**
 * Sample application demonstrating Chroma vector search with Genkit.
 *
 * <p>Indexes a handful of film descriptions into a Chroma collection, retrieves the nearest matches
 * for a query, and answers questions with a RAG flow. Requires a running Chroma server (see the
 * README for a one-line Docker command) and a {@code GEMINI_API_KEY}.
 */
public class ChromaRAGSample {

  private static final Logger logger = LoggerFactory.getLogger(ChromaRAGSample.class);
  ...
  public static void main(String[] args) {
    ...
    String chromaUrl = getEnvOrDefault(dotenv, "CHROMA_URL", "http://localhost:8000");
    String collection = getEnvOrDefault(dotenv, "CHROMA_COLLECTION", "genkit_films");

    logger.info("Starting Chroma RAG Sample (url={}, collection={})", chromaUrl, collection);

    ChromaPlugin chromaPlugin =
    ...
    String action = "chroma/" + collection;
    ...
    logger.info(
        "Genkit Chroma RAG Sample started. Flows: indexDocuments, retrieveDocuments, ragQuery");
