 ...
 *
 * <p>This example shows how to: - Configure Genkit with the Anthropic plugin - Define flows - Use
 * tools with Claude 3.5 - Generate text with Claude models - Use streaming for real-time responses
 * - Expose flows via HTTP endpoints
 *
 * <p>To run: 1. Set the ANTHROPIC_API_KEY environment variable 2. Run: mvn exec:java
 */
public class AnthropicSample {
  ...
  public static void main(String[] args) throws Exception {
              ...
              ModelResponse response =
                  genkit.generateStream(
                      GenerateOptions.builder()
                          .model("anthropic/claude-sonnet-4-5-20250929")
                          .system(
                              "You are Claude, a helpful assistant that provides detailed, comprehensive responses.")
                          .prompt(userMessage)
    ...
    System.out.println(
        "  POST http://localhost:8080/api/flows/streamingWeather (streaming + tools)");
    System.out.println("  POST http://localhost:8080/api/flows/generateCode (code generation)");
    System.out.println(
        "  POST http://localhost:8080/api/flows/creativeWriting (creative streaming)");
