...
package com.google.genkit.samples.postgresql;
...
/**
 * Sample application demonstrating PostgreSQL pgvector search with Genkit.
 *
 * <p>
 * This sample shows how to:
 * <ul>
 * <li>Configure PostgreSQL plugin with pgvector</li>
 ...
 * <li>Running PostgreSQL instance with pgvector extension</li>
 * <li>Optional: POSTGRES_HOST, POSTGRES_PORT, POSTGRES_DB, POSTGRES_USER,
 * POSTGRES_PASSWORD</li>
 * </ul>
 */
public class PostgresRAGSample {

  private static final Logger logger = LoggerFactory.getLogger(PostgresRAGSample.class);
  ...
  public static void main(String[] args) {
    ...
    String pgHost = getEnvOrDefault(dotenv, "POSTGRES_HOST", "localhost");
    String pgPort = getEnvOrDefault(dotenv, "POSTGRES_PORT", "5432");
    String pgDb = getEnvOrDefault(dotenv, "POSTGRES_DB", "genkit");
    String pgUser = getEnvOrDefault(dotenv, "POSTGRES_USER", "postgres");
    String pgPassword = getEnvOrDefault(dotenv, "POSTGRES_PASSWORD", "postgres");

    String connectionString = String.format("jdbc:postgresql://%s:%s/%s", pgHost, pgPort, pgDb);

    logger.info("Starting PostgreSQL RAG Sample");
    logger.info("PostgreSQL Connection: {}", connectionString);

    // Create Jetty plugin
    JettyPlugin jetty = new JettyPlugin(JettyPluginOptions.builder().port(8081).build());

    // Build PostgreSQL plugin
    PostgresPlugin postgresPlugin = PostgresPlugin.builder().connectionString(connectionString).username(pgUser)
        .password(pgPassword).hikariProperty("maximumPoolSize", 10).hikariProperty("minimumIdle", 2)
        .addTable(PostgresTableConfig.builder().tableName("films").embedderName("googleai/text-embedding-004")
            .vectorDimension(768).distanceStrategy(PostgresTableConfig.DistanceStrategy.L2)
            .createTableIfNotExists(true).createIndexIfNotExists(true).indexLists(100).build())
        .build();

    // Build Genkit with PostgreSQL plugin
    Genkit genkit = Genkit.builder().options(GenkitOptions.builder().devMode(true).reflectionPort(3100).build())
        .plugin(GoogleGenAIPlugin.create(apiKey)).plugin(postgresPlugin).plugin(jetty).build();
    ...
    Flow<Void, String, Void> indexDocumentsFlow = genkit.defineFlow("indexDocuments", Void.class, String.class,
        (ctx, input) -> {
          ...
          genkit.index("postgresql/films", documents);
    ...
    Flow<String, List<String>, Void> retrieveDocumentsFlow = genkit.defineFlow("retrieveDocuments", String.class,
        (Class<List<String>>) (Class<?>) List.class, (ctx, query) -> {
          ...
          List<Document> docs = genkit.retrieve("postgresql/films", query);
    ...
    Flow<String, String, Void> ragQueryFlow = genkit.defineFlow("ragQuery", String.class, String.class,
        (ctx, question) -> {
          ...
          List<Document> docs = genkit.retrieve("postgresql/films", question);
    ...
    logger.info("Genkit PostgreSQL RAG Sample Started");
    logger.info("=".repeat(60));
    logger.info("");
