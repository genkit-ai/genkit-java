 ...
 *
 * <p>This example shows: - Using @JsonPropertyDescription for field descriptions -
 * Using @JsonProperty(required = true) for required fields - Generating typed objects with
 * genkit.generate() - Using flows with typed inputs and outputs - Using tools with typed inputs and
 * outputs - Loading prompts from dotprompt files
 *
 * <p>All examples are exposed as flows that can be called via HTTP when running with Jetty plugin.
 */
public class StructuredOutputSample {
  ...
  private static void defineFlows(Genkit genkit) {
    ...
    Tool<RecipeRequest, MenuItem> recipeGenerator =
        genkit.defineTool(
            "generateRecipe",
              ...
              GenerateOptions.<MenuItem>builder()
                  ...
                  .prompt(
                      "I want a "
                          + request.getCuisine()
                          + " recipe"
                          + (request.getDietaryRestrictions() != null
                                  && !request.getDietaryRestrictions().isEmpty()
