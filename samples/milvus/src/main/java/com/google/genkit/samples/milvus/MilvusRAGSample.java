...
package com.google.genkit.samples.milvus;
...
/**
 * Sample application demonstrating Milvus vector search with Genkit.
 *
 * <p>Indexes a handful of film descriptions into a Milvus collection, retrieves the nearest matches
 * for a query, and answers questions with a RAG flow. Requires a running Milvus server (see the
 * README for a one-line Docker command) and a {@code GEMINI_API_KEY}.
 */
public class MilvusRAGSample {

  private static final Logger logger = LoggerFactory.getLogger(MilvusRAGSample.class);
  ...
  public static void main(String[] args) {
    ...
    String milvusUrl = getEnvOrDefault(dotenv, "MILVUS_URL", "http://localhost:19530");
    String milvusToken = getEnv(dotenv, "MILVUS_TOKEN"); // optional
    String collection = getEnvOrDefault(dotenv, "MILVUS_COLLECTION", "genkit_films");

    logger.info("Starting Milvus RAG Sample (url={}, collection={})", milvusUrl, collection);

    MilvusPlugin milvusPlugin =
    ...
    JettyPlugin jetty = new JettyPlugin(JettyPluginOptions.builder().port(8088).build());
    Genkit genkit =
        Genkit.builder()
    ...
    String action = "milvus/" + collection;
    ...
    logger.info(
        "Genkit Milvus RAG Sample started. Flows: indexDocuments, retrieveDocuments, ragQuery");
