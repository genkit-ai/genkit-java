 ...
 *
 * <p>This example shows how to: - Configure Genkit with the AWS Bedrock plugin - Use models with
 * ON_DEMAND support - Use models with inference profiles for cross-region inference - Define flows
 * - Use tools with models - Generate text with various Bedrock models - Use streaming for real-time
 * responses - Expose flows via HTTP endpoints
 *
 * <p>To run: 1. Configure AWS credentials (environment variables, ~/.aws/credentials, or IAM role)
 * 2. Ensure you have access to the models in AWS Bedrock console 3. For inference profile models,
 * enable cross-region inference in Bedrock console 4. Run: ./run.sh or mvn exec:java
 */
public class AwsBedrockSample {
  ...
  public static void main(String[] args) throws Exception {
    ...
    Flow<String, String, Void> compareModelsFlow =
        genkit.defineFlow(
            ...
            (ctx, question) -> {
              ...
              ModelResponse novaLiteResponse =
                  genkit.generate(
                      GenerateOptions.builder()
                          .model("aws-bedrock/amazon.nova-lite-v1:0")
                          .prompt(question)
                          .build());
              result
                  .append("Amazon Nova Lite:\n")
                  .append(novaLiteResponse.getText())
                  .append("\n\n");

              // Meta Llama response
              ModelResponse llamaResponse =
                  genkit.generate(
                      GenerateOptions.builder()
    ...
    Flow<String, String, Void> inferenceProfileFlow =
        genkit.defineFlow(
            "inferenceProfileDemo",
            String.class,
            String.class,
            (ctx, prompt) -> {
