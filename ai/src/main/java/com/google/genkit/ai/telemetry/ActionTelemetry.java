 ...
 *
 * <ul>
 *   <li>Request counts per action type
 *   <li>Latency histograms per action
 *   <li>Failure counts
 * </ul>
 */
public class ActionTelemetry {
  ...
  private ActionTelemetry() {
    ...
    requestCounter =
        meter
            ...
            .setDescription("Counts calls to genkit actions.")
            .setUnit("1")
            .build();
    ...
    latencyHistogram =
        meter
            ...
            .setDescription("Latencies when executing Genkit actions.")
            .setUnit("ms")
            .ofLongs()
            .build();
   ...
   * @param actionType the action type (flow, model, tool, etc.)
   * @param featureName the feature name (flow name or action name)
   * @param path the span path
   * @param latencyMs the latency in milliseconds
   * @param error the error name if failed, null otherwise
   */
  public void recordActionMetrics(
      String actionName,
      String actionType,
      String featureName,
      String path,
    ...
    Attributes baseAttrs =
        Attributes.builder()
            .put("name", truncate(actionName, 1024))
            .put("type", actionType != null ? actionType : "unknown")
