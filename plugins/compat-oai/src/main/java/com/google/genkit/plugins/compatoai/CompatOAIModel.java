...
public class CompatOAIModel implements Model {
  ...
  private static final Logger logger = LoggerFactory.getLogger(CompatOAIModel.class);
  ...
  public ModelResponse run(ActionContext context, ModelRequest request) {
    try {
      ...
      throw new GenkitException("API call failed", e);
    }
  }
  ...
  public ModelResponse run(ActionContext context, ModelRequest request, Consumer<ModelResponseChunk> streamCallback) {
    ...
    try {
      return callAPIStreaming(request, streamCallback);
    } catch (Exception e) {
      throw new GenkitException("Streaming API call failed", e);
    }
  }
  ...
  private ModelResponse callAPI(ModelRequest request) throws IOException {
    ...
    try (Response response = client.newCall(httpRequest).execute()) {
      if (!response.isSuccessful()) {
        ...
        throw new GenkitException("API error: " + response.code() + " - " + errorBody);
      }
  ...
  private ObjectNode buildRequestBody(ModelRequest request) {
    ...
    if (output != null && output.getFormat() == OutputFormat.JSON) {
      ...
      for (JsonNode messageNode : messages) {
        ...
        if (messageContent != null && messageContent.toLowerCase().contains("json")) {
          hasJsonKeyword = true;
          break;
        }
      }

      // If no JSON keyword found, add it to the first user message
      if (!hasJsonKeyword && messages.size() > 0) {
        for (int i = 0; i < messages.size(); i++) {
          JsonNode messageNode = messages.get(i);
          if ("user".equals(messageNode.get("role").asText())) {
            JsonNode contentNode = messageNode.get("content");
          ...
          if (toolCallsNode != null && toolCallsNode.isArray()) {
            for (JsonNode toolCallNode : toolCallsNode) {
              ...
              if (functionNode != null) {
                ...
                if (argsNode != null) {
                  @SuppressWarnings("unchecked")
                  Map<String, Object> args = objectMapper.readValue(argsNode.asText(), Map.class);
                  toolRequest.setInput(args);
                }
              }
