...
/**
 * Integration tests for agent HTTP serving in the Jetty plugin.
 *
 * <p>Verifies the one-turn-per-request transport for {@code ActionType.AGENT} bidi actions: a
 * non-streaming {@code {data, init}} request, an SSE streaming request, and a companion {@code
...
class AgentHttpServingTest {
   ...
   * companion {@code agent-snapshot} action, then starts a JettyPlugin on the given port.
   */
