...
/**
 * Integration test verifying the abort companion endpoint over the HTTP wire using JettyPlugin.
 *
 * <p>Cross-referenced with {@code Agent.abort(String)} / {@code AgentActions.buildAbortAction}: the
 * abort companion only flips a stored snapshot's status from {@code PENDING} to {@code ABORTED}; it
 * never interrupts a running foreground {@code AgentFn} call (there is no cancellation hook
 * threaded into a synchronous {@code AgentFn.run}). This test therefore targets what is actually
 * implemented: aborting a snapshot that is genuinely in the {@code PENDING} window opened by a
 * detached turn (see {@code DetachController}), which synchronously persists the {@code PENDING}
 * row before the HTTP response returns and whose finalizer never overwrites an {@code ABORTED}
 * status (race-safe by design) — then confirms via {@code getSnapshot} that the status is {@code
 * ABORTED} and stays that way even after the background turn body completes.
 */
