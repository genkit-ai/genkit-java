 ...
 * the client actually reach a served {@code AgentFn} through {@code JettyPlugin}'s {@code
 * AgentHandler} / {@code CompanionHandler}.
 *
 ...
 * {@code JettyPlugin} never read incoming HTTP headers on the server side, so a tool/flow had no
 * way to observe them. These tests start a real Jetty server, send a real HTTP request (via {@link
 * RemoteAgent}/{@link RemoteAgentOptions#headers()}) carrying a custom header, and assert a
...
class AgentHeaderPropagationTest {
  ...
  /**
   * Starts a Jetty server on {@code port} hosting a server-managed, no-model custom agent named
   * {@code headerEchoAgent}. The agent's {@code AgentFn} reads {@code
   * fnCtx.context().getContext().get("headers")} (the reserved key that {@code JettyPlugin} merges
   * incoming HTTP headers into) and echoes the requested header's value back in its reply text, so
   * the test can assert on it without any model call.
   */
   ...
   * RemoteAgentOptions.Builder#headers(Map)} sends a real HTTP POST to a live Jetty server; the
