...
/**
 * Integration test verifying the detach feature works over the HTTP wire using SpringPlugin.
 *
 * <p>Mirrors {@code plugins/jetty}'s {@code AgentDetachHttpTest}. Proves that a DETACH turn works
 * through the Spring agent HTTP endpoint: POST a turn with {@code detach:true} → server returns
 * {@code finishReason: "detached"} + a pending {@code snapshotId} immediately → the background work
 * finalizes → polling the {@code getSnapshot} companion shows {@code status: "completed"} with the
 * accumulated state.
 */
