...
/**
 * Integration tests for {@link RemoteAgent} / {@link HttpAgentTransport} against a live Spring
 * server running a server-managed echo agent.
 *
 * <p>Mirrors {@code plugins/jetty}'s {@code RemoteAgentTest}, proving that the same {@code
 * com.google.genkit.client.RemoteAgent} / {@code AgentChat} client used against a Jetty server also
 * works unmodified against a {@link SpringPlugin}-backed server — i.e. true cross-plugin wire
 * compatibility for the agent HTTP contract.
 *
 * <p>Placed in the {@code plugins/spring} module because that module depends on {@code genkit}
 * (which contains {@code RemoteAgent}) and can also start a {@link SpringPlugin} — giving us both
...
class RemoteAgentTest {
  ...
  /**
   * Starts a Spring server on {@code port} hosting a server-managed echo agent named {@code
   * echoAgent}.
   *
   * <p>The agent:
   *
  ...
  /**
   * Starts a Spring server on {@code port} hosting a server-managed custom agent named {@code
  ...
  /**
   * Starts a Spring server on {@code port} hosting a client-managed (no store) custom agent named
