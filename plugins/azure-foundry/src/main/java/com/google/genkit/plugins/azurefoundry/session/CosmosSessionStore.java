...
/**
 * Azure Cosmos DB-backed implementation of {@link SessionStore} and {@link SnapshotSubscriber}.
 *
 ...
 * SnapshotSharding}.
 *
 * <h3>Storage layout (single container)</h3>
 *
 * <p>All documents live in one container (default database {@code genkit}, container {@code
 * genkit-sessions}) partitioned by {@code /pk} (the per-tenant prefix, default {@code "global"}).
 * Each document's {@code id} discriminates the record kind:
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>Cosmos document ids may not contain {@code #}, so the layout uses {@code _} separators; ids
 * are only ever constructed and looked up (never parsed back into components).
 *
 * <h3>Concurrency</h3>
 *
 * <p>Snapshot and shard documents are idempotent by id (upserted); the snapshot document uses ETag
 * ({@code If-Match}) optimistic concurrency when updating an existing id, re-applying the (pure)
 * mutator on {@code 412}. The session pointer is advanced monotonically (never backward) under ETag
 * concurrency. Shards are written before the snapshot document, which is written before the pointer
 * flips, so a reader following the pointer always sees complete data.
 *
 * @param <S> the type of custom session state
 */
public final class CosmosSessionStore<S> implements SessionStore<S>, SnapshotSubscriber {

  private static final Logger logger = LoggerFactory.getLogger(CosmosSessionStore.class);
   ...
   */
  public CosmosSessionStore(CosmosClient client) {
    this(client, CosmosSessionStoreOptions.defaults());
  }
   ...
   */
  public CosmosSessionStore(CosmosClient client, CosmosSessionStoreOptions options) {
    if (client == null) {
      throw new IllegalArgumentException("CosmosClient must be non-null");
    }
    if (options == null) {
    ...
    if (options.isCreateIfNotExists()) {
      client.createDatabaseIfNotExists(options.getDatabaseName());
      CosmosDatabase database = client.getDatabase(options.getDatabaseName());
      database.createContainerIfNotExists(options.getContainerName(), "/pk");
      this.container = database.getContainer(options.getContainerName());
    } else {
      this.container =
          client.getDatabase(options.getDatabaseName()).getContainer(options.getContainerName());
    }
    this.scheduler =
        Executors.newSingleThreadScheduledExecutor(
            r -> {
              Thread t = new Thread(r, "genkit-cosmos-session-store-poll");
              t.setDaemon(true);
              return t;
            });
  }
  ...
  public String saveSnapshot(
    ...
    PartitionKey pk = new PartitionKey(prefix);

    for (int attempt = 1; ; attempt++) {
      // 1. Read existing snapshot (+ etag) and reconstruct state.
      SessionSnapshot<S> existing = null;
      String existingSessionId = null;
      String existingEtag = null;
      if (snapshotId != null) {
        SnapshotSharding.validateId(snapshotId);
        CosmosItemResponse<ObjectNode> resp = readItemOrNull(pk, snapId(snapshotId));
        if (resp != null) {
          ObjectNode item = resp.getItem();
          existing = readSnapshot(prefix, item);
          existingSessionId = existing.getSessionId();
          existingEtag = resp.getETag();
        }
      }
      ...
      if (parentId != null && !parentId.isBlank()) {
        ObjectNode parentDoc = readDoc(pk, snapId(parentId));
        if (parentDoc != null) {
          parent = loadParentInfo(prefix, parentDoc);
        }
      }
      ...
      ObjectNode doc = baseDoc(prefix, result);
      String checkpointId;
      ...
      if (checkpoint) {
        ...
        for (int i = 0; i < shards.size(); i++) {
          ...
          shardDoc.put("id", shardId(checkpointId, i));
          shardDoc.put("pk", prefix);
          shardDoc.put("checkpointId", checkpointId);
          shardDoc.put("index", i);
          shardDoc.put("data", shards.get(i));
          container.upsertItem(shardDoc, pk, new CosmosItemRequestOptions());
        }
      ...
      try {
        if (existingEtag == null) {
          ...
          container.replaceItem(
              ...
              snapId(finalId),
              pk,
              new CosmosItemRequestOptions().setIfMatchETag(existingEtag));
        }
      } catch (CosmosException e) {
        if ((e.getStatusCode() == STATUS_CONFLICT
                || e.getStatusCode() == STATUS_PRECONDITION_FAILED)
            && attempt < MAX_ATTEMPTS) {
          continue; // concurrent writer won the race; re-read and retry the pure mutator.
        }
        throw new GenkitException("Failed to save snapshot: " + e.getMessage(), e);
      }
  ...
  private void advancePointer(
    ...
    for (int attempt = 1; ; attempt++) {
      ...
      String etag = null;
      CosmosItemResponse<ObjectNode> resp = readItemOrNull(pk, id);
      if (resp != null) {
        existing = resp.getItem();
        etag = resp.getETag();
        String currentLeaf = getString(existing, "currentSnapshotId");
        String currentCreatedAt = getString(existing, "currentCreatedAt");
      ...
      try {
        if (existing == null) {
          container.createItem(ptr, pk, new CosmosItemRequestOptions());
        } else {
          container.replaceItem(ptr, id, pk, new CosmosItemRequestOptions().setIfMatchETag(etag));
        }
        return;
      } catch (CosmosException e) {
        boolean conflict =
            e.getStatusCode() == STATUS_CONFLICT || e.getStatusCode() == STATUS_PRECONDITION_FAILED;
        if (conflict && attempt < MAX_ATTEMPTS) {
          continue; // concurrent pointer write; re-read and re-evaluate never-backward.
        }
        if (conflict) {
          logger.debug("Pointer for session {} not advanced (lost concurrency race)", sessionId);
          return;
        }
        throw new GenkitException("Failed to advance session pointer: " + e.getMessage(), e);
      }
    }
  }
  ...
  public SessionSnapshot<S> getSnapshot(GetSnapshotOptions opts) {
    ...
    if (opts.getSnapshotId() != null) {
      ...
      ObjectNode doc = readDoc(pk, snapId(opts.getSnapshotId()));
      return doc == null ? null : readSnapshot(prefix, doc);
    }
    if (opts.getSessionId() != null) {
      SnapshotSharding.validateId(opts.getSessionId());
      ObjectNode pointer = readDoc(pk, ptrId(opts.getSessionId()));
      if (pointer == null) {
        return null;
      }
      ...
      ObjectNode doc = readDoc(pk, snapId(leafId));
      return doc == null ? null : readSnapshot(prefix, doc);
    }
    return null;
  }
  ...
  /** Builds the non-state base document for a snapshot (id/pk + metadata). */
  private ObjectNode baseDoc(String prefix, SessionSnapshot<S> snap) {
    ObjectNode data = MAPPER.createObjectNode();
    data.put("id", snapId(snap.getSnapshotId()));
    data.put("pk", prefix);
  ...
  private JsonNode reconstructFullState(
    ...
    for (int i = 0; i < checkpointShardCount; i++) {
      ObjectNode shard = readDoc(pk, shardId(checkpointId, i));
    ...
    for (String diffId : segmentPath) {
      ObjectNode diffDoc = readDoc(pk, snapId(diffId));
      if (diffDoc != null) {
        String patch = getString(diffDoc, "statePatch");
        if (patch != null) {
          diffs.add(patch);
        }
      }
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // Low-level Cosmos + node helpers
  // ──────────────────────────────────────────────────────────────────────────

  /** Reads an item and returns its full response, or {@code null} when it does not exist. */
  private CosmosItemResponse<ObjectNode> readItemOrNull(PartitionKey pk, String id) {
    try {
      return container.readItem(id, pk, ObjectNode.class);
    } catch (CosmosException e) {
      if (e.getStatusCode() == STATUS_NOT_FOUND) {
        return null;
      }
      throw new GenkitException("Failed to read Cosmos item " + id + ": " + e.getMessage(), e);
    }
  }

  /** Reads an item body, or {@code null} when it does not exist. */
  private ObjectNode readDoc(PartitionKey pk, String id) {
    CosmosItemResponse<ObjectNode> resp = readItemOrNull(pk, id);
