 ...
 *
 * This plugin registers Grok models as Genkit actions using the
...
public class XAIPlugin implements Plugin {
   ...
   */
  public XAIPlugin() {
    this(CompatOAIPluginOptions.builder().apiKey(getApiKeyFromEnv()).baseUrl("https://api.x.ai/v1").build());
  }
  ...
  public static XAIPlugin create(String apiKey) {
    return new XAIPlugin(CompatOAIPluginOptions.builder().apiKey(apiKey).baseUrl("https://api.x.ai/v1").build());
  }
  ...
  public List<Action<?, ?, ?>> init() {
    ...
    for (String modelName : SUPPORTED_MODELS) {
      CompatOAIModel model = new CompatOAIModel("xai/" + modelName, modelName, // API model name without prefix
