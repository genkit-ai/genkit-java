 ...
 * </ul>
 *
 * <p>Both are implemented as <em>tool factories</em>: they return {@code List<Tool<?, ?>>} (and,
 * for {@code agents()}, a system-prompt fragment) that callers wire into an agent via {@code
 * AgentConfig.tools(...)} and {@code AgentConfig.system(...)}. This avoids modifying the generate
 * pipeline while still letting a model delegate to sub-agents and read/write artifacts.
 *
